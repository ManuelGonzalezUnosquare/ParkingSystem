...
export class PasswordRecoveryConfirm {
  ...
  constructor() {
    effect(() => {
      ...
      if (cCode) {
        this.codeForm.patchValue({ code: cCode });
      }
    });
  }
  ...
  async doSubmitCode() {
    ...
    const response = await lastValueFrom(this.service.validateResetCode(code));
    if (response.data) {
  ...
  async doSubmitPassword() {
    ...
    const response = await this.store.resetPasswordConfirm({
